...
function DepositBankAcc({ open, handleClose, onSave, pid }) {
    ...
    if (amount != "" && amount != "$0.00") {
      ...
      try {
        const response = await api.post(
          `cash-account/deposit/${pid}/${amount}`
        );
        ...
        setMessage(`Error when depositing ${amount}`);
      ...
      setMessage("Please provide an amount to deposit");
      setSnackSeverity("error");
      setOpenSnackbar(true);
      return;
    }
  ...
  return (
    <div>
      <Dialog open={open} onClose={handleClose}>
        <DialogTitle>Deposit from External Bank Account</DialogTitle>
        <DialogContent>
