...
const FriendsPage = () => {
  ...
  const FindUser = async (username) => {
    try {
      ...
      const res = await api.get(`find-username/${username}`);
  ...
  const handleFriendReq = async () => {
    const friends = await api.post(`send-friend-request/${user}/`,  {
      method: "POST",});
    friends.data.message === "duplicate friends" ? alert(`already friends with ${user} `) :
  ...
  return (
    <div>
      ...
      <div>
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginLeft:'210px' }}>
              <h2>{user}</h2>
              {user && <button onClick={handleFriendReq}>SEND REQUEST</button>}
          </div>
      </div>
          ...
          {friendList && friendList.length > 0 ? (
           friendList.map((friend) => (
              <div key={friend}  style={{display:'flex', gap:'10px'}}> 
                <h2>{friend}</h2>
                <button 
                 onClick={() => handleRemoveFriend(friend)}>Remove Friend</button>
            </div>
          ))
          ) : (
          <div>No Friends</div>
      ...
      <div  style={{alignItems: 'center', gap: '10px', marginLeft: '20px', marginTop:'50px' }}>
        <h2>PENDING REQUESTS</h2>
        <div>
          {pendingList && pendingList.length > 0 ? (
          pendingList.map((friend) => (
              <div key={friend.id} style={{display:'flex', gap:'10px'}}> 
                <h2>{friend.requester_username}</h2>
                <button
                 onClick={() => handleAddFriend(friend.requester_username)}>Add Friend</button>
                 <button onClick={ () => handleRemoveFriend(friend.requester_username)}>Delete request</button>
            </div>
          ))
          ) : (
      ...
      <div  style={{alignItems: 'center', gap: '10px', marginLeft: '20px', marginTop:'50px' }}>
        <h2>OUR SENT REQUESTS</h2>
        <div>
          {ourPendingList && ourPendingList.length > 0 ? (
          ourPendingList.map((friend) => (
              <div key={friend.id} style={{display:'flex', gap:'10px'}}> 
                <h2>{friend.receiver_username}</h2>
                <button
                 onClick={() => removeSentReq(friend.receiver_username)}>Delete Request</button>
            </div>
          ))
          ) : (
